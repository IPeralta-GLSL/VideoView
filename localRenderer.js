...
async function pickSupportedCodec(width, height) {
  for (const codec of CODEC_PROFILES) {
    try {
      ...
      if (result.supported) {
        return result.config?.codec ?? codec;
      }
    } catch {}
  }
  return 'avc1.640033';
...
function drawFrame(ctx, v1, v2, label1, label2, showLabels, outW, outH) {
  ...
  ctx.fillText(label1, padX, padY + 4);
  ...
  ctx.fillStyle = '#ffffff';
  ctx.fillText(label2, x2, padY + 4);
}

async function encodeVideoPhase({
  videoBase, videoOverlay,
  startTime, endTime,
  outW, outH,
  label1, label2, showLabels,
  onProgress,
}) {
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    const videoFrame = new VideoFrame(canvas, { timestamp, duration: FRAME_DURATION_US });
...
export async function startLocalRender({
  ...
  try {
    ...
    const silentBuffer = await encodeVideoPhase({
      videoBase, videoOverlay,
      startTime, endTime,
      outW, outH,
      label1, label2, showLabels,
